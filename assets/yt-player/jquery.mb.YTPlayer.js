    ...
    buildPlayer: function(options) {
      return this.each(function() {
        ...
        var playlistID = this.opt.videoURL
          ...
          : $YTPlayer.attr("href") ? getYTPVideoID($YTPlayer.attr("href")).playlistID : false;
        ...
        YTPlayer.defaultOpt.containment = YTPlayer.opt.containment =
          YTPlayer.opt.containment == "self" ? jQuery(this) : jQuery(YTPlayer.opt.containment);
        ...
        .on("YTPData.YTPlayer", function() {
          if (YTPlayer.hasData) {
            if (YTPlayer.isPlayer && !YTPlayer.opt.autoPlay) {
              var bgndURL =
                YTPlayer.videoData.thumb_max || YTPlayer.videoData.thumb_high || YTPlayer.videoData.thumb_medium;
              YTPlayer.opt.containment.css({
          ...
          function(data) {
            ...
            function parseYTPlayer_data(data) {
              ...
              YTPlayer.videoData.description =
                data.description.length < 400 ? data.description : data.description.substring(0, 400) + " ...";
      ...
      jQuery(YTPlayer.playerEl).CSSAnimate(
        ...
        function() {
          ...
          jQuery(YTPlayer)
            .YTPGetPlayer()
            .cueVideoByUrl(
              encodeURI(jQuery.mbYTPlayer.locationProtocol + "//www.youtube.com/v/" + YTPlayer.videoID),
              1,
    ...
    fullscreen: function(real) {
      ...
      if (real) {
        var fullscreenchange = jQuery.browser.mozilla
          ? "mozfullscreenchange"
          : jQuery.browser.webkit ? "webkitfullscreenchange" : "fullscreenchange";
        jQuery(document)
      ...
      return this.each(function() {
        ...
        for (var key in YTPlayer.filters) {
          if (YTPlayer.filters[key].value)
            filterStyle +=
              key.replace("_", "-") + "(" + YTPlayer.filters[key].value + YTPlayer.filters[key].unit + ") ";
        }
      ...
      return this.each(function() {
        function o() {
          (f.called = !0),
            ...
            "function" == typeof f.CSSqueue && (f.CSSqueue(), (f.CSSqueue = null));
        }
        var f = this,
          g = jQuery(this);
        f.id = f.id || "CSSA_" + new Date().getTime();
        var h = h || { type: "noEvent" };
        if (f.CSSAIsRunning && f.eventType == h.type && !jQuery.browser.msie && jQuery.browser.version <= 9)
          return (
            (f.CSSqueue = function() {
    ...
    updateSliderVal: function(b) {
      ...
      (d.x =
        "object" == typeof b
          ...
          : "number" == typeof b ? b * e / d.opt.scale : d.opt.initialval * e / d.opt.scale),
        (d.y =
          "object" == typeof b
        ...
        (d.outOfRangeX = d.scaleX > d.opt.scale ? d.scaleX - d.opt.scale : d.scaleX < 0 ? d.scaleX : 0),
        (d.outOfRangeY = d.scaleY > d.opt.scale ? d.scaleY - d.opt.scale : d.scaleY < 0 ? d.scaleY : 0),
        (d.outOfRange = "h" == d.opt.orientation ? d.outOfRangeX : d.outOfRangeY),
        (d.value =
          "undefined" != typeof b
            ? "h" == d.opt.orientation
              ? d.x >= c.outerWidth() ? d.opt.scale : d.x <= 0 ? 0 : d.scaleX
              : d.y >= c.outerHeight() ? d.opt.scale : d.y <= 0 ? 0 : d.scaleY
            : "h" == d.opt.orientation ? d.scaleX : d.scaleY),
        "h" == d.opt.orientation ? d.level.width(g(d.x, e) + "%") : d.level.height(g(d.y, f)),
